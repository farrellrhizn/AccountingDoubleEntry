        ...
        <div className="relative">
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white border rounded-lg shadow-lg z-10">
                    <ul className="py-1">
                        {["Edit", "Delete", "Reset Password", "Login Disable"].map(option => (
                            <li
                                key={option}
...
export default function StaffUser() {
    ...
    return (
        <>
            <TitleCard title="Staff User" topMargin="mt-2" TopSideButtons={<TopSideButtons onAddUser={openAddUserModal} />}>
                {/* Adding grid with 4 columns */}
                <div className="grid grid-cols-4 gap-4">
                    {/* Iterating through dummy data STAFFUSER_DATA */}
                    {STAFFUSER_DATA.map((user, index) => (
                        <CardUser
                            ...
                            topMargin="mt-2"
                            TopSideButtonsUser={<div className="m-2"><TopSideButtonsUser /></div>}
                        >
